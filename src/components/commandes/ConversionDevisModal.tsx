...
export default function ConversionDevisModal({ devis, isOpen, onClose, onSuccess }: ConversionModalProps) {  const [loading, setLoading] = useState(false);
  ...
  const [formData, setFormData] = useState({
    ...
    temps_estime: 8,
    notes_installation: '',
    adresse_installation: devis.client?.adresse || '',
    contact_site: devis.client?.nom || '',
    telephone_contact: devis.client?.telephone || '',
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Devis {devis.numero} - {devis.client?.entreprise}
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="text-blue-700">{devis.client?.entreprise}</p>
                <p className="text-blue-600">{devis.client?.nom}</p>
              </div>
              <div>
                <span className="font-medium text-blue-800">Montant HT:</span>
              ...
              <div>
                <span className="font-medium text-blue-800">Montant TTC:</span>
                <p className="text-blue-700 text-lg font-semibold">
                  {new Intl.NumberFormat('fr-FR', {
                    style: 'currency',
                    currency: 'EUR'
